...
MethodStatus BaselineCompiler::compile() {
  ...
  if (c->err) {
    ...
    reply = (redisReply *)redisCommand(c, "SET %s %s", compilationId.c_str(), "baseline-compiler");
    printf("[BaselineCompiler::compile] SET %s %s\n", compilationId.c_str(), "baseline-compiler");
 ...
 out:
  ...
  /*
  //printf("[BaselineCompiler::compile] baseline jit code = %u\n", *(code->raw()));
  printf("[BaselineCompiler::compile] baseline jit code ptr = %p\n", code->raw());
  printf("[BaselineCompiler::compile] baseline jit code :\n");
  for (int i = 0; i < code->instructionsSize(); i++) {
    printf("%x", code->raw()[i]);
  }
  printf("\n");
  uint32_t jitCodeHash = memory_hash(code->raw(), code->instructionsSize());
  printf("[BaselineCompiler::compile] baseline jit code hash = %x\n", jitCodeHash);
  printf("\n");
  */
