...
static bool AddInlinedCompilations(JSContext* cx, HandleScript script,
  ...
  for (const auto* scriptSnapshot : snapshot->scripts()) {
    ...
    /*
    std::ostringstream inlinedScriptNameStream;
    inlinedScriptNameStream << std::string(inlinedScript->filename()) << ":" << std::to_string(inlinedScript->lineno()) << ":" << std::to_string(inlinedScript->column().oneOriginValue());
    std::string inlinedScriptName = inlinedScriptNameStream.str();
    printf("[AddInlinedCompilations] inlinedScript = %s\n", inlinedScriptName.c_str());
    */
...
/*
static uint32_t block_mix(uint32_t memory_block, uint32_t internal_state) {
   return (internal_state * memory_block) ^
          ((internal_state << 3) + (memory_block >> 2));
}

static uint32_t memory_hash(const uint8_t* memory, size_t memory_length) {
   uint32_t internal_state = 0xA5A5A5A5; // IV: A magic number
   uint32_t memory_block = 0;

   // Loop over the message 32-bits at-a-time
   while (memory_length >= sizeof(uint32_t)) {
      memcpy(&memory_block, memory, sizeof(uint32_t));

      internal_state = block_mix(memory_block, internal_state);

      memory_length -= sizeof(uint32_t);
      memory        += sizeof(uint32_t);
   }

   // Are there any remaining bytes?
   if (memory_length > 0) {
      memcpy(&memory_block, memory, memory_length);
      internal_state = block_mix(memory_block, internal_state);
   }
...
static JitCode* fail(JSContext* cx) {
  ReportOutOfMemory(cx);
  return nullptr;
}

static void linkProfilerCallSitesOnRestore(JitCode* code, Vector<CodeOffset, 0, SystemAllocPolicy>& profilerCallSites) {
  for (size_t i = 0; i < profilerCallSites.length(); i++) {
    CodeOffset offset = profilerCallSites[i];
    CodeLocationLabel location(code, offset);
    Assembler::PatchDataWithValueCheck(location, ImmPtr(location.raw()),
                            ImmPtr((void*)-1));
  }
}
...
// based on Linker::newCode, but ignores the parts using the masm, just allocate a copy at the right place and restore context
static JitCode* restoreJitCodeAndContext(JSContext* cx, Vector<CodeOffset, 0, SystemAllocPolicy>& profilerCallSites, JitCode* savedJitCode) {
  JS::AutoAssertNoGC nogc(cx);

  size_t headerSize = savedJitCode->headerSize();
  size_t bytesNeeded = headerSize + savedJitCode->bufferSize();
  ...
  // MAKE SURE THE ALLOCATED MEMORY IS NOT FREED / GARBAGE COLLECTED BEFORE THE END OF CODE EXECUTION


  ExecutablePool* pool;
  uint8_t* result =
      (uint8_t*)jitZone->execAlloc().alloc(cx, bytesNeeded, &pool, CodeKind::Ion);
  if (!result) {
    return fail(cx);
  }
  ...
  JitCode* newJitCode =
      JitCode::New<NoGC>(cx, codeStart, bytesNeeded, headerSize, pool, CodeKind::Ion);
  if (!newJitCode) {
    return fail(cx);
  }

  // make buffer writable
  //ExecutableAllocator::makeWritable(newJitCode->raw(), newJitCode->bufferSize());
  // UNCOMMENT ?

  // Store the JitCode pointer in the JitCodeHeader so we can recover the
  // gcthing from relocation tables.
  JitCodeHeader::FromExecutable(newJitCode->raw())->init(newJitCode);
  newJitCode->setInstructionsSize(savedJitCode->instructionsSize());
  //memcpy(newJitCode->raw(), savedJitCode->raw(), newJitCode->instructionsSize());
  newJitCode->setJumpRelocTableBytes(savedJitCode->jumpRelocTableBytes());
  //memcpy(newJitCode->raw() + newJitCode->jumpRelocTableOffset(), savedJitCode->raw() + savedJitCode->jumpRelocTableOffset(), newJitCode->jumpRelocTableBytes());
  newJitCode->setDataRelocTableBytes(savedJitCode->dataRelocTableBytes());
  //memcpy(newJitCode->raw() + newJitCode->dataRelocTableOffset(), savedJitCode->raw() + savedJitCode->dataRelocTableOffset(), newJitCode->dataRelocTableBytes());
  memcpy(newJitCode->raw(), savedJitCode->raw(), newJitCode->bufferSize()); // memcopy everything directly instead

  (void)linkProfilerCallSitesOnRestore;
  //linkProfilerCallSitesOnRestore(newJitCode, profilerCallSites);
  // UNCOMMENT WHEN WE WILL RESTORE FROM REDIS

  if (savedJitCode->hasBytecodeMap())
    newJitCode->setHasBytecodeMap();

  uint32_t savedJitCodeHash = JSScript::memoryHash((uint8_t *)&savedJitCode, sizeof(JitCode));
  uint32_t newJitCodeHash = JSScript::memoryHash((uint8_t *)&newJitCode, sizeof(JitCode));

  uint32_t savedJitCodeBufferHash = JSScript::memoryHash(savedJitCode->raw(), savedJitCode->bufferSize());
  uint32_t newJitCodeBufferHash = JSScript::memoryHash(newJitCode->raw(), newJitCode->bufferSize());

  printf("\n");
  printf("\n");
  printf("\n");
  printf("bytesNeeded = %lu\n", bytesNeeded);
  printf("\n");
  printf("savedJitCode = %p\n", savedJitCode);
  printf("savedJitCode->raw() = %p\n", savedJitCode->raw());
  printf("savedJitCode->instructionsSize() = %lu\n", savedJitCode->instructionsSize());
  printf("savedJitCode->jumpRelocTableOffset() = %u\n", savedJitCode->jumpRelocTableOffset());
  printf("savedJitCode->dataRelocTableOffset() = %u\n", savedJitCode->dataRelocTableOffset());
  printf("savedJitCode->jumpRelocTableBytes() = %u\n", savedJitCode->jumpRelocTableBytes());
  printf("savedJitCode->dataRelocTableBytes() = %u\n", savedJitCode->dataRelocTableBytes());
  printf("savedJitCode->hasBytecodeMap() = %d\n", savedJitCode->hasBytecodeMap());
  printf("savedJitCode->headerSize() = %lu\n", savedJitCode->headerSize());
  printf("savedJitCode->bufferSize() = %lu\n", savedJitCode->bufferSize());
  printf("memoryHash(&savedJitCode, sizeof(JitCode)) = %u\n", savedJitCodeHash);
  printf("memoryHash(savedJitCode->raw(), savedJitCode->bufferSize()) = %u\n", savedJitCodeBufferHash);
  printf("\n");
  printf("newJitCode = %p\n", newJitCode);
  printf("newJitCode->raw() = %p\n", newJitCode->raw());
  printf("newJitCode->instructionsSize() = %lu\n", newJitCode->instructionsSize());
  printf("newJitCode->jumpRelocTableOffset() = %u\n", newJitCode->jumpRelocTableOffset());
  printf("newJitCode->dataRelocTableOffset() = %u\n", newJitCode->dataRelocTableOffset());
  printf("newJitCode->jumpRelocTableBytes() = %u\n", newJitCode->jumpRelocTableBytes());
  printf("newJitCode->dataRelocTableBytes() = %u\n", newJitCode->dataRelocTableBytes());
  printf("newJitCode->hasBytecodeMap() = %d\n", newJitCode->hasBytecodeMap());
  printf("newJitCode->headerSize() = %lu\n", newJitCode->headerSize());
  printf("newJitCode->bufferSize() = %lu\n", newJitCode->bufferSize());
  printf("memoryHash(&newJitCode, sizeof(JitCode)) = %u\n", newJitCodeHash);
  printf("memoryHash(newJitCode->raw(), newJitCode->bufferSize()) = %u\n", newJitCodeBufferHash);
...
void CodeGenerator::dumpIonScript(IonScript* ionScript, size_t ionScriptSize, size_t ionScriptSideDataSize) {
  printf("\n");

  // dump ion script
  printf("[dumpIonScript]\n");

  // dump ion script size
  printf("dumping ion script size : %lu\n", ionScriptSize);

  // dump ion script side data size
  printf("dumping ion script side data size : %lu\n", ionScriptSideDataSize);

  /*
  // dump constants
  printf("dumping constants :\n");
  for (size_t i = 0; i < ionScript->numConstants(); i++) {
    const Value* vp = ionScript->constants();
    const Value& v = vp[i];
    printf("%d : %lu\n", i, v.asRawBits());
  }

  // dump runtime data
  printf("dumping runtime data hash :\n");
  uint32_t runtimeDataHash = memory_hash(ionScript->runtimeData(), ionScript->runtimeSize());
  printf("runtime data hash : %u\n", runtimeDataHash);

  // dump nursery objects
  printf("dumping nursery objects :\n");
  for (size_t i = 0; i < ionScript->numNurseryObjects(); i++) {
    NurseryObjectVector ionScript->nurseryObjects()[i];
  }

  // dump OSI indices
  printf("dumping OSI indices :\n");
  for (size_t i = 0; i < ionScript->numOsiIndices(); i++) {
    ionScript->osiIndices()[i];
  }

  // dump safepoint indices
  printf("dumping safepoint indices :\n");
  for (size_t i = 0; i < ionScript->numSafepointIndices(); i++) {
    ionScript->safepointIndices()[i];
  }

  // dump IC indices
  printf("dumping IC indices\n");
  for (size_t i = 0; i < ionScript->numICs(); i++) {
    ionScript->icIndex()[i];
  }

  // dump safepoints
  printf("dumping safepoints :\n");
  for (size_t i = 0; i < ionScript->safepointsSize(); i++) {
    ionScript->safepoints()[i];
  }

  // dump snapshots
  printf("dumping snapshots :\n");
  for (size_t i = 0; i < ionScript->snapshotListSize() + ionScript->snapshotsRVATableSize(); i++) {
    ionScript->snapshots();
  }

  // dump recovers
  printf("dumping recovers :\n");
  for (size_t i = 0; i < ionScript->recoversSize(); i++) {
    ionScript->recovers();
  }
  */
...
bool CodeGenerator::link(JSContext* cx, const WarpSnapshot* snapshot) {
  ...
  //printf("[CodeGenerator::link] Linking code\n");
  ...
  IonScript* ionScript = IonScript::New(
      ...
      safepoints_.size(), &ionScriptSize);
  if (!ionScript) {
    return false;
  }
#ifdef DEBUG
  ...
  (void)ionScriptSize;
  //printf("[CodeGenerator::link] ionScriptSize = %lu\n", ionScriptSize);
  ...
  JitCode* code;
  if (!copyCode) {
    code = linker.newCode(cx, CodeKind::Ion);
  } else {
    JitCode* codeAlt = linker.newCode(cx, CodeKind::Ion);
    Vector<CodeOffset, 0, SystemAllocPolicy>& profilerCallSites = masm.profilerCallSites();
    code = restoreJitCodeAndContext(cx, profilerCallSites, codeAlt);
  }
  ...
  // DISABLE REDIS ION EAGER STORE
  bool disableRedisEagerStore = false;
  if (disableRedisEagerStore)
    goto out2;
  ...
  if (c->err) {
    ...
    std::ostringstream compilationIdStream;
    uint32_t bytecodeHash = script->bytecodeHash();
    compilationIdStream << "[bytecodeHash=" << std::to_string(bytecodeHash) << "]" << std::string(script->filename()) << ":" << std::to_string(script->lineno()) << ":" << std::to_string(script->column().oneOriginValue());
    std::string compilationId = compilationIdStream.str();

    //reply = (redisReply *)redisCommand(c, "EXISTS %s", compilationId.c_str());
    //printf("[CodeGenerator::link] EXISTS %s -> %lld\n", compilationId.c_str(), reply->integer);

    reply = (redisReply *)redisCommand(c, "SET %s %s", compilationId.c_str(), "ion-compiler");
    printf("[CodeGenerator::link] SET %s %s\n", compilationId.c_str(), "ion-compiler");
 ...
 out2:
  ...
  // DISABLE REDIS JIT CODE STORE
  bool disableRedisStore = true;
  if (disableRedisStore)
    goto out;

  // TO REMOVE
  dumpIonScript(ionScript, ionScriptSize, 0);

  // Saving jited code to redis
  c = redisConnect("127.0.0.1", 6379);
  if (c->err) {
    printf("error: %s\n", c->errstr);
  } else {
    ...
    TempAllocator alloc(&cx->tempLifoAlloc());

    // Call Sites
    // std::stringstream callSitesKeyStream;
    // callSitesKeyStream << functionId << "[callSites]";
    // std::string callSitesKey = callSitesKeyStream.str();


    // JitCode
    std::stringstream jitCodeKeyStream;
    jitCodeKeyStream << compilationId << ",jitCode";
    std::string jitCodeKey = jitCodeKeyStream.str();

    reply = (redisReply *)redisCommand(c, "SET %s %b", jitCodeKey.c_str(), (char *)code, sizeof(JitCode));
    freeReplyObject(reply);

    // reply = (redisReply *)redisCommand(c, "GET %s", jitCodeKey.c_str());
    // uint32_t jitCodeHash = memory_hash((uint8_t *)code, sizeof(JitCode));
    // uint32_t jitCodeRedisHash = memory_hash((uint8_t *)reply->str, reply->len);
    // printf("jitCodeHash = %u\n", jitCodeHash);
    // printf("jitCodeRedisHash = %u\n", jitCodeRedisHash);
    //freeReplyObject(reply);


    // JitCodeHeader
    std::stringstream jitCodeHeaderKeyStream;
    jitCodeHeaderKeyStream << compilationId << ",jitCodeHeader";
    std::string jitCodeHeaderKey = jitCodeHeaderKeyStream.str();
    JitCodeHeader * codeHeader = (JitCodeHeader *)((uint8_t *)(code->raw()) - code->headerSize());

    reply = (redisReply *)redisCommand(c, "SET %s %b", jitCodeHeaderKey.c_str(), (char *)codeHeader, code->headerSize());
    freeReplyObject(reply);

    //reply = (redisReply *)redisCommand(c, "GET %s", jitCodeHeaderKey.c_str());
    // uint32_t jitCodeHeaderHash = memory_hash((uint8_t *)codeHeader, code->headerSize());
    // uint32_t jitCodeHeaderRedisHash = memory_hash((uint8_t *)reply->str, reply->len);
    // printf("jitCodeHeaderHash = %u\n", jitCodeHeaderHash);
    // printf("jitCodeHeaderRedisHash = %u\n", jitCodeHeaderRedisHash);
    //freeReplyObject(reply);


    // Binary Code
    std::stringstream binaryCodeKeyStream;
    binaryCodeKeyStream << compilationId << ",binaryCode";
    std::string binaryCodeKey = binaryCodeKeyStream.str();

    reply = (redisReply *)redisCommand(c, "SET %s %b", binaryCodeKey.c_str(), (char *)code->raw(), code->bufferSize());
    //printf("REQUEST: SET %s %s\n", binaryCodeKey.c_str(), "code->raw()");
    //printf("RESPONSE: NONE\n");
    freeReplyObject(reply);

    //reply = (redisReply *)redisCommand(c, "GET %s", binaryCodeKey.c_str());
    // uint32_t binaryCodeHash = memory_hash((uint8_t *)code->raw(), code->bufferSize());
    // uint32_t binaryCodeRedisHash = memory_hash((uint8_t *)reply->str, reply->len);
    // printf("binaryCodeHash = %u\n", binaryCodeHash);
    // printf("binaryCodeRedisHash = %u\n", binaryCodeRedisHash);
    //freeReplyObject(reply);


    // IonScript
    std::stringstream ionScriptKeyStream;
    ionScriptKeyStream << compilationId << ",ionScript";
    std::string ionScriptKey = ionScriptKeyStream.str();

    reply = (redisReply *)redisCommand(c, "SET %s %b", ionScriptKey.c_str(), (char *)ionScript, ionScriptSize);
    freeReplyObject(reply);

    //reply = (redisReply *)redisCommand(c, "GET %s", ionScriptKey.c_str());
    // uint32_t ionScriptHash = memory_hash((uint8_t *)ionScript, ionScriptSize);
    // uint32_t ionScriptRedisHash = memory_hash((uint8_t *)reply->str, reply->len);
    // printf("ionScriptHash = %u\n", ionScriptHash);
    // printf("ionScriptRedisHash = %u\n", ionScriptRedisHash);
    //freeReplyObject(reply);


    // Offsets
    std::stringstream extraObjectsKeyStream;
    extraObjectsKeyStream << compilationId << ",extraObjects";
    std::string extraObjectsKey = extraObjectsKeyStream.str();

    size_t invalidateOffset = 0;
    size_t invalidateEpilogueDataOffset = invalidateOffset + sizeof(Label);
    size_t ionScriptLabelsLengthOffset = invalidateEpilogueDataOffset + sizeof(CodeOffset);
    size_t ionScriptLabelsOffset = ionScriptLabelsLengthOffset + sizeof(size_t);
    size_t ionNurseryObjectLabelsLengthOffset = ionScriptLabelsOffset + sizeof(CodeOffset) * ionScriptLabels_.length();
    size_t ionNurseryObjectLabelsOffset = ionNurseryObjectLabelsLengthOffset + sizeof(size_t);
    size_t icInfoLengthOffset = ionNurseryObjectLabelsOffset + sizeof(NurseryObjectLabel) * ionNurseryObjectLabels_.length();
    size_t icInfoOffset = icInfoLengthOffset + sizeof(size_t);
    size_t extraObjectsSize = icInfoOffset + sizeof(CompileTimeICInfo) * icInfo_.length();

    uint8_t* extraObjects = (uint8_t*)alloc.allocateInfallible(extraObjectsSize);

    size_t ionScriptLabelsNr = ionScriptLabels_.length();
    size_t ionNurseryObjectLabelsNr = ionNurseryObjectLabels_.length();
    size_t icInfoSize = icInfo_.length();

    memcpy(extraObjects + invalidateOffset, &invalidate_, sizeof(Label));
    memcpy(extraObjects + invalidateEpilogueDataOffset, &invalidateEpilogueData_, sizeof(CodeOffset));
    memcpy(extraObjects + ionScriptLabelsLengthOffset, &ionScriptLabelsNr, sizeof(size_t));
    int i = 0;
    for (CodeOffset offset : ionScriptLabels_) {
      memcpy(extraObjects + ionScriptLabelsOffset + i * sizeof(CodeOffset), &offset, sizeof(CodeOffset));
      i++;
    }
    memcpy(extraObjects + ionNurseryObjectLabelsLengthOffset, &ionNurseryObjectLabelsNr, sizeof(size_t));
    i = 0;
    for (NurseryObjectLabel label : ionNurseryObjectLabels_) {
      memcpy(extraObjects + ionNurseryObjectLabelsOffset + i * sizeof(NurseryObjectLabel), &label, sizeof(NurseryObjectLabel));
      i++;
    }
    memcpy(extraObjects + icInfoLengthOffset, &icInfoSize, sizeof(size_t));
    i = 0;
    for (CompileTimeICInfo info : icInfo_) {
      memcpy(extraObjects + icInfoOffset + i * sizeof(CompileTimeICInfo), &info, sizeof(CompileTimeICInfo));
      i++;
    }

    reply = (redisReply *)redisCommand(c, "SET %s %b", extraObjectsKey.c_str(), (char *)extraObjects, extraObjectsSize);
    freeReplyObject(reply);
    ...
    printf("[CodeGenerator::link] Stored compilation data in redis\n");
    printf("[CodeGenerator::link] compilationId = %s\n", compilationId.c_str());
  }
