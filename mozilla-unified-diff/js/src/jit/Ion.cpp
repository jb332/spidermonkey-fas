...
IonScript* IonScript::New(JSContext* cx, IonCompilationId compilationId,
                          ...
                          size_t safepointsSize, size_t* allocationSize) {
  ...
  if (allocationSize)
    *allocationSize = allocSize.value();
...
/*
// recreate an IonScript from a saved IonScript
IonScript* IonScript::New(JSContext* cx, IonCompilationId compilationId,
                          uint32_t localSlotsSize, uint32_t argumentSlotsSize,
                          uint32_t frameSize, size_t snapshotsListSize,
                          size_t snapshotsRVATableSize, size_t recoversSize,
                          ...
                          size_t safepointIndices, size_t osiIndices,
                          size_t icEntries, size_t runtimeSize,
                          size_t safepointsSize, IonScript* savedIonScript) {
  if (snapshotsListSize >= MAX_BUFFER_SIZE) {
    ReportOutOfMemory(cx);
    return nullptr;
  }
  ...
  IonScript* script = new (raw)
      IonScript(savedIonScript->compilationId(), savedIonScript->localSlotsSize(), savedIonScript->argumentSlotsSize(), savedIonScript->frameSize());
  ...
  script->nurseryObjectsOffset_ = savedIonScript->nurseryObjectsOffset();
  script->initElements<HeapPtr<JSObject*>>(script->nurseryObjectsOffset(), nurseryObjects);
  script->osiIndexOffset_ = savedIonScript->osiIndexOffset();
  script->safepointIndexOffset_ = savedIonScript->safepointIndexOffset();
  script->icIndexOffset_ = savedIonScript->icIndexOffset();
  script->safepointsOffset_ = savedIonScript->safepointsOffset();
  script->snapshotsOffset_ = savedIonScript->snapshotsOffset();
  script->rvaTableOffset_ = savedIonScript->rvaTableOffset();
  script->recoversOffset_ = savedIonScript->recoversOffset();
  script->allocBytes_ = savedIonScript->allocBytes();
...
static JitCode* restoreJitCode(JSContext* cx, JitCode* savedJitCode, uint8_t* savedBinaryCode) {
  JS::AutoAssertNoGC nogc(cx);

  size_t headerSize = savedJitCode->headerSize();
  size_t bytesNeeded = headerSize + savedJitCode->bufferSize();

  printf("[restoreJitCode] Restoring JitCode and context\n");
  ...
  memcpy(newJitCode->raw(), savedBinaryCode, newJitCode->bufferSize());

  if (savedJitCode->hasBytecodeMap())
    newJitCode->setHasBytecodeMap();

  /*
  uint32_t savedJitCodeHash = memory_hash((uint8_t *)savedJitCode, sizeof(JitCode));
  uint32_t newJitCodeHash = memory_hash((uint8_t *)newJitCode, sizeof(JitCode));
  // they are different because of raw

  uint32_t savedBinaryCodeHash = memory_hash(savedBinaryCode, savedJitCode->bufferSize());
  uint32_t newBinaryCodeHash = memory_hash(newJitCode->raw(), newJitCode->bufferSize());
  // they are the same

  printf("[restoreJitCode] savedJitCodeHash = %u\n", savedJitCodeHash);
  printf("[restoreJitCode] newJitCodeHash = %u\n", newJitCodeHash);

  printf("[restoreJitCode] savedBinaryCodeHash = %u\n", savedBinaryCodeHash);
  printf("[restoreJitCode] newBinaryCodeHash = %u\n", newBinaryCodeHash);
  */

  return newJitCode;
}

static IonScript* restoreIonScript(JSContext* cx, IonScript* ionScript) {
  JS::AutoAssertNoGC nogc(cx);

  size_t ionScriptSize;

  IonScript* newIonScript = IonScript::New(
      cx, ionScript->compilationId(), ionScript->localSlotsSize(), ionScript->argumentSlotsSize(), ionScript->frameSize(),
      ionScript->snapshotsListSize(), ionScript->snapshotsRVATableSize(), ionScript->recoversSize(),
      ionScript->numConstants(), ionScript->numNurseryObjects(), ionScript->numSafepointIndices(),
      ionScript->numOsiIndices(), ionScript->numICs(), ionScript->runtimeSize(),
      ionScript->safepointsSize(), &ionScriptSize);

  size_t ionScriptSideDataSize = ionScript->numConstants() +
      ionScript->runtimeSize() +
      ionScript->numNurseryObjects() +
      ionScript->numOsiIndices() +
      ionScript->numSafepointIndices() +
      ionScript->numICs() +
  ...
  printf("[restoreIonScript] ionScriptSideDataSize = %lu\n", ionScriptSideDataSize);

  // TO REMOVE
  CodeGenerator::dumpIonScript(ionScript, ionScriptSize, ionScriptSideDataSize);

  memcpy(newIonScript->constants(), ionScript->constants(), ionScriptSideDataSize);

  if (ionScript->hasProfilingInstrumentation())
    newIonScript->setHasProfilingInstrumentation();

  return newIonScript;
}

static bool linkFromRedis(JSContext* cx, JSScript* scriptPtr, jsbytecode* osrPc, JitCode* savedJitCode, uint8_t* savedBinaryCode, IonScript* ionScript, uint8_t* extraObjects) {
  JS::AutoAssertNoGC nogc(cx);
  TempAllocator alloc(&cx->tempLifoAlloc());

  RootedScript script(cx, scriptPtr);
  MOZ_ASSERT(!script->hasIonScript());
  ...
  IonScript* newIonScript = restoreIonScript(cx, ionScript);

  auto freeIonScript = mozilla::MakeScopeExit([&newIonScript] {
    // Use js_free instead of IonScript::Destroy: the cache list is still
    // uninitialized.
    js_free(newIonScript);
  });


  JitCode* newJitCode = restoreJitCode(cx, savedJitCode, savedBinaryCode);


  // // Encode native to bytecode map if profiling is enabled.
  // if (newIonScript->hasProfilingInstrumentation()) {
  //   // Generate native-to-bytecode main table.
  //   IonEntry::ScriptList scriptList;
  //   if (!generateCompactNativeToBytecodeMap(cx, newJitCode, scriptList)) {
  //     return false;
  //   }
  ...
  //       cx, newJitCode, newJitCode->raw(), newJitCode->rawEnd(), std::move(scriptList), ionTable);
  //   if (!entry) {
  //     return false;
  //   }
  ...
  //   auto entry = MakeJitcodeGlobalEntry<DummyEntry>(cx, newJitCode, newJitCode->raw(),
  //                                                   newJitCode->rawEnd());
  //   if (!entry) {
  //     return false;
  //   }
  ...
  newIonScript->setMethod(newJitCode);
  ...
  size_t ionScriptLabelsNr;
  memcpy(&ionScriptLabelsNr, extraObjects + ionScriptLabelsLengthOffset, sizeof(size_t));

  size_t ionScriptLabelsOffset = ionScriptLabelsLengthOffset + sizeof(size_t);
  size_t ionNurseryObjectLabelsLengthOffset = ionScriptLabelsOffset + sizeof(CodeOffset) * ionScriptLabelsNr;

  size_t ionNurseryObjectLabelsNr;
  memcpy(&ionNurseryObjectLabelsNr, extraObjects + ionNurseryObjectLabelsLengthOffset, sizeof(size_t));

  size_t ionNurseryObjectLabelsOffset = ionNurseryObjectLabelsLengthOffset + sizeof(size_t);
  size_t icInfoLengthOffset = ionNurseryObjectLabelsOffset + sizeof(CodeGenerator::NurseryObjectLabel) * ionNurseryObjectLabelsNr;

  size_t icInfoSize;
  memcpy(&icInfoSize, extraObjects + icInfoLengthOffset, sizeof(size_t));

  size_t icInfoOffset = icInfoLengthOffset + sizeof(size_t);

  size_t extraObjectsSize = icInfoOffset + sizeof(CodeGenerator::CompileTimeICInfo) * icInfoSize;

  CodeOffset invalidateEpilogueData;
  memcpy(&invalidateEpilogueData, extraObjects + invalidateEpilogueDataOffset, sizeof(CodeOffset));
  Assembler::PatchDataWithValueCheck(
      CodeLocationLabel(newJitCode, invalidateEpilogueData), ImmPtr(newIonScript),
      ImmPtr((void*)-1));

  for (int i = 0; i < ionScriptLabelsNr; i++) {
    CodeOffset offset;
    memcpy(&offset, extraObjects + ionScriptLabelsOffset + i * sizeof(CodeOffset), sizeof(CodeOffset));
    Assembler::PatchDataWithValueCheck(CodeLocationLabel(newJitCode, offset),
                                       ImmPtr(ionScript), ImmPtr((void*)-1));
  }

  for (int i = 0; i < ionNurseryObjectLabelsNr; i++) {
    CodeGenerator::NurseryObjectLabel* label = (CodeGenerator::NurseryObjectLabel*)alloc.allocateInfallible(sizeof(CodeGenerator::NurseryObjectLabel));
    memcpy(label, extraObjects + ionNurseryObjectLabelsOffset + i * sizeof(CodeGenerator::NurseryObjectLabel), sizeof(CodeGenerator::NurseryObjectLabel));
    void* entry = ionScript->addressOfNurseryObject(label->nurseryIndex);
    Assembler::PatchDataWithValueCheck(CodeLocationLabel(newJitCode, label->offset),
                                       ImmPtr(entry), ImmPtr((void*)-1));
  }

  for (size_t i = 0; i < icInfoSize; i++) {
    CodeGenerator::CompileTimeICInfo compileTimeIcInfo;
    memcpy(&compileTimeIcInfo, extraObjects + icInfoOffset + i * sizeof(CodeGenerator::CompileTimeICInfo), sizeof(CodeGenerator::CompileTimeICInfo));
    IonIC& ic = ionScript->getICFromIndex(i);
    Assembler::PatchDataWithValueCheck(
        CodeLocationLabel(newJitCode, compileTimeIcInfo.icOffsetForJump),
        ImmPtr(ic.codeRawPtr()), ImmPtr((void*)-1));
    Assembler::PatchDataWithValueCheck(
        CodeLocationLabel(newJitCode, compileTimeIcInfo.icOffsetForPush), ImmPtr(&ic),
        ImmPtr((void*)-1));
  }

  newIonScript->setInvalidationEpilogueDataOffset(
      invalidateEpilogueData.offset());
  if (osrPc) {
    newIonScript->setOsrPc(osrPc);
    newIonScript->setOsrEntryOffset(newIonScript->osrEntryOffset());
  }
  Label invalidate;
  memcpy(&invalidate, extraObjects + invalidateOffset, sizeof(Label));
  newIonScript->setInvalidationEpilogueOffset(invalidate.offset());


  // TO DO
  freeIonScript.release();

  script->jitScript()->setIonScript(script, newIonScript);

  ExecutableAllocator::makeExecutableAndFlushICache(newJitCode->raw(), newJitCode->bufferSize());
...
static MethodStatus Compile(JSContext* cx, HandleScript script,
  ...
  // DISABLE REDIS JIT CODE RESTORE
  bool disableRedisRestore = true;
  if (disableRedisRestore)
    goto out;

  // Restoring jited code from redis
  redisReply *reply;
  redisContext *c;
  ...
  if (c->err) {
    ...
    JitCode* jitCode = (JitCode*)alloc.allocateInfallible(reply->len);
    memcpy(jitCode, reply->str, reply->len);
    //uint32_t jitCodeRedisHash = JSScript::memoryHash((uint8_t *)reply->str, reply->len);
    freeReplyObject(reply);
    ...
    uint8_t* binaryCode = (uint8_t*)alloc.allocateInfallible(reply->len);
    memcpy(binaryCode, reply->str, reply->len);
    //uint32_t binaryCodeRedisHash = JSScript::memoryHash((uint8_t *)reply->str, reply->len);
    freeReplyObject(reply);
    ...
    IonScript* ionScript = (IonScript*)alloc.allocateInfallible(reply->len);
    //size_t ionScriptSize = reply->len;
    memcpy(ionScript, reply->str, reply->len);
    //uint32_t ionScriptRedisHash = JSScript::memoryHash((uint8_t *)reply->str, reply->len);
    freeReplyObject(reply);
    ...
    reply = (redisReply *)redisCommand(c, "GET %s", extraObjectsKey.c_str());
    if (!reply->len) {
      freeReplyObject(reply);
      goto out;
    }
    uint8_t* extraObjects = (uint8_t*)alloc.allocateInfallible(reply->len);
    //size_t extraObjectsSize = reply->len;
    memcpy(extraObjects, reply->str, reply->len);
    //uint32_t extraObjectsRedisHash = JSScript::memoryHash((uint8_t *)reply->str, reply->len);
    freeReplyObject(reply);
    ...
    printf("[Compile] Retrieving compiled code from Redis database\n");

    linkFromRedis(cx, script, osrPc, jitCode, binaryCode, ionScript, extraObjects);
...
MethodStatus jit::CanEnterIon(JSContext* cx, RunState& state) {
  ...
  if ((JitOptions.eagerIonCompilation() /*|| script->ionEager()*/) && !script->hasBaselineScript()) {
    MethodStatus status =
  ...
  // skip compilation if no baseline
  ...
  /*
  std::ostringstream scriptNameStream;
  scriptNameStream << std::string(script->filename()) << ":" << std::to_string(script->lineno()) << ":" << std::to_string(script->column().oneOriginValue());
  std::string scriptName = scriptNameStream.str();
  printf("\n-> script = %s\n", scriptName.c_str());
  printf("[CanEnterIon] Requesting compilation\n");
  */
...
static MethodStatus BaselineCanEnterAtEntry(JSContext* cx, HandleScript script,
  ...
  //printf("[BaselineCanEnterAtEntry] Compiling code\n");
...
static MethodStatus BaselineCanEnterAtBranch(JSContext* cx, HandleScript script,
  ...
  //printf("[BaselineCanEnterAtBranch] Compiling code\n");
...
bool jit::IonCompileScriptForBaselineOSR(JSContext* cx, BaselineFrame* frame,
  ...
  *infoPtr = nullptr;
  ...
  /*
  std::ostringstream scriptNameStream;
  scriptNameStream << std::string(frame->script()->filename()) << ":" << std::to_string(frame->script()->lineno()) << ":" << std::to_string(frame->script()->column().oneOriginValue());
  std::string scriptName = scriptNameStream.str();
  printf("\n-> script = %s\n", scriptName.c_str());
  */
  ...
  /*
  printf("[IonCompileScriptForBaselineOSR] jitcode = %p\n", jitcode);
  printf("[IonCompileScriptForBaselineOSR] info = %p\n", info);
  printf("[IonCompileScriptForBaselineOSR] Jumping into compiled code\n");
  */
