...
EnterJitStatus js::jit::MaybeEnterJit(JSContext* cx, RunState& state) {
  ...
  do {
    ...
    if (jit::IsIonEnabled(cx)) {
      jit::MethodStatus status = jit::CanEnterIon(cx, state); // skip compilation if no baseline
      if (status == jit::Method_Error) {
        printf("[MaybeEnterJit] Ion error\n");
        return EnterJitStatus::Error;
      }
      if (status == jit::Method_Compiled) {
        code = script->jitCodeRaw();
        printf("[MaybeEnterJit] Ion code compiled\n");
        break;
      }
    }
    ...
    if (jit::IsBaselineJitEnabled(cx)) {
      ...
      if (status == jit::Method_Error) {
        printf("[MaybeEnterJit] Baseline compiler error\n");
        return EnterJitStatus::Error;
      }
      if (status == jit::Method_Compiled) {
        code = script->jitCodeRaw();
        printf("[MaybeEnterJit] Baseline compiler code compiled\n");
        break;
      }
    }
    ...
    if (IsBaselineInterpreterEnabled()) {
      ...
      if (status == jit::Method_Error) {
        printf("[MaybeEnterJit] Baseline interpreter error\n");
        return EnterJitStatus::Error;
      }
      if (status == jit::Method_Compiled) {
        code = script->jitCodeRaw();
        printf("[MaybeEnterJit] Baseline interpreter code compiled\n");
        break;
      }
    }
