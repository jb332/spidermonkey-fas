...
static uint32_t blockMix(uint32_t memory_block, uint32_t internal_state) {
  return (internal_state * memory_block) ^
         ((internal_state << 3) + (memory_block >> 2));
}

uint32_t JSScript::memoryHash(const uint8_t* memory, size_t memory_length) {
  ...
  while (memory_length >= sizeof(uint32_t)) {
    ...
    internal_state = blockMix(memory_block, internal_state);
...
uint32_t JSScript::bytecodeHash() {
  return JSScript::memoryHash((uint8_t *)this->code(), this->length());
}
